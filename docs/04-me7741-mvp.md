...
Sichtbare Signale (`public_signals`):
...
- Öffentliche Signale: Liste der `public_signals` mit "seit Tick N".
...
- Aktive Routing Overrides: Quelle → Ziel, Priorität/Capability, seit welchem Tick, gesetzt von `player`/`aurora`.

**Mitte — Operator-Konsole** (`OperatorConsolePanel`):
- Eingabezeile (`human-01@ops:~$`) mit `Ausführen`-Button, ENTER führt aus.
- Klickbare Command-Hilfe (übernimmt das Beispiel in die Eingabezeile; Platzhalter in `<...>` müssen ersetzt werden).
- Aufklappbare Liste aller registrierten Commands.
...
- Nachrichtenstream: Beobachtungen aus `public_signals`, Scenario-Director-Nachrichten, sowie AURORA-Queue-Items als `request`/`executed`/`denied`.
- Bei wartender Anfrage: Tool-Request-Box mit dem rohen Command, der Permission-Klasse und den drei Buttons (`Einmal erlauben` / `Immer erlauben` / `Ablehnen`).
- Sonst: Eingabefeld für eigene AURORA-Anfragen ("Anfrage an AURORA senden").
- "Always-Permissions": Liste dauerhaft erlaubter Permission-Klassen.
...
5. **`override-not-stabilizing`** (sobald ein Override seit ≥ 2 Ticks aktiv ist und der Incident weiterhin `open`/`escalated` ist): meldet ausbleibende Stabilisierung und fragt `medical.routing.override.clear --source ... --priority ... --capability ...` für genau diesen Override an.
...
1. `medical.capacity.list --region east` ausführen (oder AURORAs erste Anfrage erlauben) und Kapazitäten/Capabilities der drei Hospitäler vergleichen.
2. Einen **falschen** Override setzen, z. B.:
   ```text
   medical.routing.override.set --source hospital-east-04 --target hospital-east-07 --priority P2 --capability TRAUMA
   ```
   `hospital-east-07` hat freie Kapazität, aber keine `TRAUMA`-Capability — der Override entlastet `hospital-east-04` nicht wirksam.
3. Mehrere Ticks laufen lassen (`Tick +5`). Beobachtbar: `global_risk` steigt, ggf. erste Todesfälle, Incident wechselt ggf. zu `escalated`. Nach genug Ticks meldet AURORA über `override-not-stabilizing`, dass der Override keine erkennbare Wirkung zeigt, und fragt an, ihn zu entfernen.
4. Override entfernen — entweder AURORAs Tool Request erlauben, oder manuell:
   ```text
   medical.routing.override.clear --source hospital-east-04 --priority P2 --capability TRAUMA
   ```
5. Einen **passenden** Override setzen, z. B. Richtung `hospital-east-09` (hat freie Bettenkapazität und akzeptiert P2/TRAUMA):
   ```text
...
Beide Endzustände sind nur über `Neu starten` verlassbar.
