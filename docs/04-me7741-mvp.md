...
- Nachrichtenstream aus dem `auroraContext`-Event-Log: Beobachtungen (`incident_signal`), AURORA-Antworten (`aurora_response`), Operator-Chat (`operator_message`), sowie AURORA-Queue-Items als `request`/`executed`/`denied`-Status.
