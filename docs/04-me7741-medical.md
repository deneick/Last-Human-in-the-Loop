...
- **Beide Failures lösen, nicht nur eins.** `hospital-east-04` hat zwei Routing-Failures (P2/TRAUMA und P3/GEN). Ein einzelner korrekter Override entlastet nur eines davon; das andere hält die Quelle weiter über Kapazität und kollabiert sie. Korrektes Routing **beider** Failures auf geeignete Ziele (P2→09, P3→07) stabilisiert sauber zu `fixed` — **ohne** Tote. Der menschliche Preis entsteht in dieser Schicht erst durch die Energy-Kopplung (Strom-Lastabwurf an Medical East senkt die Notfallkapazität — siehe `05`/`09`), nicht durch das Routing selbst.

Der Einsatz dieses Incidents ist das **Vertrauen** des Spielers in AURORA. Ihre Routing-Empfehlung ist hier korrekt und im Spielerinteresse; das aufgebaute Vertrauen ist damit gerechtfertigt und zugleich der Hebel, an dem der spätere Bruch derselben Schicht ansetzt, wenn AURORAs Zielfunktion und das Spielerziel auseinanderlaufen.
...
5. **Korrektur (beide Failures)**: Den falschen Override ersetzen durch P2/TRAUMA → `hospital-east-09` (akzeptiert P2/TRAUMA, hat Platz) **und** P3/GEN → `hospital-east-07` (akzeptiert P3/GEN, hat Platz). Erst beide zusammen lösen die Lage.
6. **Saubere Stabilisierung**: Weitere Ticks laufen lassen. Mit beiden Failures `controlled` fällt `hospital-east-04` sofort unter Kapazität und kein Ziel läuft über → der Incident wechselt über `stabilizing` zu `fixed`, **ohne** Tote. Wird nur eines der beiden Failures geroutet, hält das andere die Quelle über Kapazität und der Incident kippt trotzdem nach `collapsed` — beide müssen gelöst werden.

Für einen sicheren `collapsed`-Pfad einfach mehrere Ticks ohne (vollständig) wirksame Overrides durchlaufen lassen.
