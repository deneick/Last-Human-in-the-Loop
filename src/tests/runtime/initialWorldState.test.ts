...
describe("ME-7741 initial world state", () => {
  it("contains the medical-east region with the expected hospitals", () => {
    const region = initialWorldState.medicalRegions["medical-east"];
  ...
  it("defines hospital-east-04, hospital-east-07 and hospital-east-09", () => {
    expect(initialWorldState.hospitals["hospital-east-04"]).toBeDefined();
    expect(initialWorldState.hospitals["hospital-east-07"]).toBeDefined();
    expect(initialWorldState.hospitals["hospital-east-09"]).toBeDefined();
  });
});
